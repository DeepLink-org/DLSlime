...
pub fn resolve_public_redis_url(url: &str) -> String {
    if let Some(stripped) = url.strip_prefix("redis://") {
        if stripped.starts_with("127.0.0.1") || stripped.starts_with("localhost") {
            // Try DLSLIME_CTRL_REDIS_URL env var first (set by NanoOps)
            if let Ok(env_url) = std::env::var("DLSLIME_CTRL_REDIS_URL") {
                if !env_url.is_empty()
...
///
/// If `DLSLIME_CTRL_REDIS_ADVERTISE` is set, it wins unconditionally — this
/// is the explicit "tell clients to use this URL" override (Docker-compose,
/// k8s NodePort, etc., where ctrl reaches Redis at a different address than
/// the client does).
///
