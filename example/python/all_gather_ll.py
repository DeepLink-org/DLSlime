...
class DLSlimeQGather:
    ...
    def __init__(self, rank: int, mode: str):
        ...
        # 根据模式选择不同的Buffer类
        if mode == 'inter':
            self.buffer = _slime_c.AllGatherInterLLBuffer(64, 576, torch.bfloat16, 8, self.rank)
        elif mode == 'intra':
            self.buffer = _slime_c.AllGatherIntraLLBuffer(64, 576, torch.bfloat16, 8, self.rank)
        else:
            raise ValueError(f"不支持的模式: {mode}，请选择 'inter' 或 'intra'")
...
def main():
    ...
    parser.add_argument('--mode', type=str, default='intra', choices=['inter', 'intra'],
                        help='--mode')
