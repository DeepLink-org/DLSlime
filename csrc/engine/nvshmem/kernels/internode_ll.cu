...
__global__ void send_ll_kernel(int8_t* data, int8_t* buffer, size_t length, size_t msg_size_per_warp, size_t num_warps_per_sm, int rank, int dst_rank) {
...
__global__ void recv_ll_kernel(int8_t* data, int8_t* buffer, size_t length, size_t msg_size_per_warp, size_t num_warps_per_sm, int rank, int src_rank) {
...
void send_ll(int8_t* data, int8_t* buffer, size_t length, size_t msg_size_per_warp, size_t num_warps_per_sm, int rank, int dst_rank) {
    size_t alignment = msg_size_per_warp * num_warps_per_sm;
    size_t block_size = (length + alignment - 1) / alignment;
    size_t thread_size = num_warps_per_sm * 32;
    SLIME_LOG_INFO("block_size: " << block_size << ", thread_size: " << thread_size << ".");
    send_ll_kernel<<<block_size, thread_size>>>(data, buffer, length, msg_size_per_warp, num_warps_per_sm, rank, dst_rank);
}

void recv_ll(int8_t* data, int8_t* buffer, size_t length, size_t msg_size_per_warp, size_t num_warps_per_sm, int rank, int src_rank) {
    size_t alignment = msg_size_per_warp * num_warps_per_sm;
    size_t block_size = (length + alignment - 1) / alignment;
    size_t thread_size = num_warps_per_sm * 32;
    SLIME_LOG_INFO("block_size: " << block_size << ", thread_size: " << thread_size << ".");
    recv_ll_kernel<<<block_size, thread_size>>>(data, buffer, length, msg_size_per_warp, num_warps_per_sm, rank, src_rank);
}
